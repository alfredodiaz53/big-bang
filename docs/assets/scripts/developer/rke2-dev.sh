...
# install rke2 on instance
echo "Installing rke2 on instance"
run "sudo curl -sfL https://get.rke2.io --output /tmp/install.sh && sudo chmod 755 /tmp/install.sh && sudo INSTALL_RKE2_METHOD=tar /tmp/install.sh"
echo
echo "rke2 version"
run "rke2 --version"
echo
...
if [[ "$PRIVATE_IP" == true ]]; then
  $sed_gsed -i "s/127\.0\.0\.1/${PrivateIP}/g" ~/.kube/${AWSUSERNAME}-dev-config
else  # default is to use public ip
  $sed_gsed -i "s/127\.0\.0\.1/${PublicIP}/g" ~/.kube/${AWSUSERNAME}-dev-config
fi
