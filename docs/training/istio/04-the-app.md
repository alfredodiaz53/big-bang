...
- The `customers` service exposes a [REST](https://restfulapi.net/) endpoint that returns a list of customers in JSON format.  The `web-frontend` calls `customers` to retrieve the list, which it uses to render to HTML.
...
1. Study the two Kubernetes yaml files: `web-frontend.yaml` and `customers.yaml`.
    ...
    ??? tldr "web-frontend.yaml"
        ```yaml linenums="1"
        --8<-- "docs/docs/training/istio/artifacts/web-frontend.yaml"
        ```

    ??? tldr "customers.yaml"
        ```yaml linenums="1"
        --8<-- "docs/docs/training/istio/artifacts/customers.yaml"
        ```
