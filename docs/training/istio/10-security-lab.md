...
In the [observability](07-observability.md#kiali) lab, we looked at the Kiali dashboard and noted the lock icons indicating that traffic was secured with mTLS.
...
1. Apply the following peer authentication policy.
    ...
    ??? tldr "mtls-strict.yaml"
        ```yaml linenums="1"
        --8<-- "docs/docs/training/istio/artifacts/mtls-strict.yaml"
        ```
...
!!! tldr "authz-policy-customers.yaml"
    ```yaml linenums="1"
    --8<-- "docs/docs/training/istio/artifacts/authz-policy-customers.yaml"
    ```
...
- Test you can still reach the `web-frontend` service
??? tldr "hint"
    ```{.shell .language-shell}
...
In the next section we learn how to use Istio's traffic management features.
